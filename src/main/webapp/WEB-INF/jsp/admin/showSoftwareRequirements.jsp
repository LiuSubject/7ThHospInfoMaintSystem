				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${softwareRequirementsList}" var="item">
								<tr <c:if test="${item.faultUrgent == 1}">class="Urgent"</c:if>>
									<td>
										<c:if test="${item.requireType == 0}">
											流程变更
										</c:if>
										<c:if test="${item.requireType == 1}">
											功能变更
										</c:if>
										<c:if test="${item.requireType == 2}">
											应用出错
										</c:if>
										<c:if test="${item.requireType == 3}">
											系统升级
										</c:if>
										<c:if test="${item.requireType == 4}">
											功能新增
										</c:if>
										<c:if test="${item.requireType == 5}">
											数据维护
										</c:if>
									</td>
									<td>${item.dept}</td>
